...
缩进有利有弊。好处是强迫你写出格式化的代码，但没有规定缩进是几个空格还是Tab。按照约定俗成的惯例，应该始终坚持使用_4个空格_的缩进。

缩进的另一个好处是强迫你写出缩进较少的代码，你会倾向于把一段很长的代码拆分成若干函数，从而得到缩进较少的代码。

缩进的坏处就是“复制－粘贴”功能失效了，这是最坑爹的地方。当你重构代码时，粘贴过去的代码必须重新检查缩进是否正确。此外，IDE很难像格式化Java代码那样格式化Python代码。

最后，请务必注意，Python程序是_大小写敏感_的，如果写错了大小写，程序会报错。
...
![step-1.png](https://tu.2644536256.date/file/blog/wengzhang/1779606476954_step-1.png)

执行`b = a`，解释器创建了变量`b`，并把`b`指向`a`指向的字符串`'ABC'`：

![step-2.png](https://tu.2644536256.date/file/blog/wengzhang/1779606490680_step-2.png)

执行`a = 'XYZ'`，解释器创建了字符串'XYZ'，并把`a`的指向改为`'XYZ'`，但`b`并没有更改：

![step-3.png](https://tu.2644536256.date/file/blog/wengzhang/1779606493118_step-3.png)
...
其实就是tuple里面添加了一个list，可变的list，本质tuple还是不变的。
...
# 总结
数据类型：整型，浮点型，字符串型，布尔型
python是弱类型语言，不用int a=10; b=3.14; name='yitong' "yitong"
‘’‘多行代码注释’‘’      Ture/False   首字母大写     And Or Not
运算符   + - * / %  > < >= <= ==
SyntaxError 语法错误 
TypeError类型错误
类型：
list：列表    name=['','','','',],append ,pop ,insert
tuple：特殊list
dict：字典    map{key：value,......}  key不能重复，重复值会覆盖
set：只有key   map{key，key，key}  没有value ，不能重复，随机存放
